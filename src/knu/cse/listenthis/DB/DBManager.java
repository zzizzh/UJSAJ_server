...
public class DBManager {
	...
	public User getUserByIndex(int index) throws Exception {
		...
		if (cursorId.hasNext()) {
			...
			if (check != null) {
				 ...
				 * locQuery.put("index", index); try { // 내컴퓨터에서 받는거 File file =
				 * new
				 * File("C:\\Users\\안준영\\Desktop\\DB사진\\프로필사진\\" + user.getUserIndex() + "
				 * .png"); InputStream is = new FileInputStream(file); long
				 * length = file.length();
				 * 
				 * byte[] bytes = new byte[(int) length]; int offset = 0; int
				 * numRead = 0; while (offset < bytes.length && (numRead =
				 * is.read(bytes, offset, bytes.length - offset)) >= 0) { offset
				 * += numRead; } user.setIImage(bytes); if (offset <
				 * bytes.length) { throw new
				 * IOException("Could not completely read file " +
				 * file.getName()); } is.close();
				 * 
	...
	public Posts getPostsByIndex(int index) throws Exception {
		...
		if (cursorId.hasNext()) {
			...
			if (check != null) {
				...
				/*
				 * try { File file = new
				 * File("C:\\Users\\안준영\\Desktop\\DB사진\\" + posts.getPostsIndex() + "
				 * .png"); InputStream is = new FileInputStream(file); long
				 * length = file.length();
				 * 
				 * byte[] bytes = new byte[(int) length]; int offset = 0; int
				 * numRead = 0; while (offset < bytes.length && (numRead =
				 * is.read(bytes, offset, bytes.length - offset)) >= 0) { offset
				 * += numRead; } posts.setIImage(bytes); if (offset <
				 * bytes.length) { throw new
				 * IOException("Could not completely read file " +
				 * file.getName()); } is.close();
				 * 
	...
	public ArrayList<Posts> getPostsByLocation(int[] location, int size) throws Exception {
		...
		/*
		 * if (i.size() < 10) { BasicDBObject squery = new BasicDBObject();
		 * 
		 * squery.put("ContentID", new BasicDBObject("$ne",
		 * location.getContentID())); squery.put("SmallLocation",
		 * location.getSmallLocation());
		 * 
		 * DBCursor cursor = locationCollection.find(squery); while
		 * (cursor.hasNext() && i.size() < 10) { int index = (int)
		 * cursor.next().get("index"); i.add(index); } } if (i.size() < 10) {
		 * BasicDBObject mquery = new BasicDBObject();
		 * 
		 * mquery.put("ContentID", new BasicDBObject("$ne",
		 * location.getContentID())); mquery.put("SmallLocation", new
		 * BasicDBObject("$ne", location.getSmallLocation()));
		 * mquery.put("MidLocation", location.getMidLocation());
		 * 
		 * DBCursor mcursor = locationCollection.find(mquery); while
		 * (mcursor.hasNext() && i.size() < 10) { int index = (int)
		 * mcursor.next().get("index"); i.add(index); } } if (i.size() < 10) {
		 * BasicDBObject bquery = new BasicDBObject();
		 * 
		 * bquery.put("ContentID", new BasicDBObject("$ne",
		 * location.getContentID())); bquery.put("SmallLocation", new
		 * BasicDBObject("$ne", location.getSmallLocation()));
		 * bquery.put("MidLocation", new BasicDBObject("$ne",
		 * location.getMidLocation())); bquery.put("BigLocation",
		 * location.getBigLocation());
		 * 
		 * DBCursor bcursor = locationCollection.find(bquery); while
		 * (bcursor.hasNext() && i.size() < 10) { int index = (int)
		 * bcursor.next().get("index"); i.add(index); } }
		 */
